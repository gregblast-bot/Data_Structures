    ...
    NodeType* treePtr, FunctionType visit)
{
   if (treePtr != 0) {
	...
	nodes.push_back(treePtr);
	
		while (!nodes.empty()) {
		
			// visit last node in nodes
			NodeType* currentNode = nodes.back();
			nodes.pop_back();
			visit(currentNode->item);
			
			// get children of nodes, add right first then left
			if (currentNode->rightChildPtr != nullptr) {
				nodes.push_back(currentNode->rightChildPtr);
			}
			
			if (currentNode->leftChildPtr != nullptr) {
				nodes.push_back(currentNode->leftChildPtr);
			}
		}
		
		/*NodeType* currentLeftNode = treePtr->leftChildPtr;
		NodeType* currentRightNode = treePtr->rightChildPtr;
		visit(treePtr->item);
		preorder(currentLeftNode, visit);
		preorder(currentRightNode, visit);*/
   }
};
    ...
    NodeType* treePtr, FunctionType visit)
{
   if (treePtr != 0) {
	   ...
	   std::vector<NodeType*> nodes;
	   nodes.push_back(treePtr);
	   NodeType* currentNode = treePtr;
 
		while (!nodes.empty()) {
			
			while (currentNode != nullptr && currentNode->leftChildPtr != nullptr) {
				nodes.push_back(currentNode->leftChildPtr);
				currentNode = currentNode->leftChildPtr;
			}
			
			visit(nodes.back()->item);
			currentNode = nodes.back()->rightChildPtr;
		...
		/*NodeType* currentLeftNode = treePtr->leftChildPtr;
		NodeType* currentRightNode = treePtr->rightChildPtr;
		inorder(currentLeftNode, visit);
		visit(treePtr->item);
		inorder(currentRightNode, visit);*/
   }
};
...
void BinaryTree<TreeItemType, FunctionType>::postorder(
    NodeType* treePtr, FunctionType visit)
{
	// Wow, this is way fucking easier
    if (treePtr != 0) {
		NodeType* currentLeftNode = treePtr->leftChildPtr;
		NodeType* currentRightNode = treePtr->rightChildPtr;
		postorder(currentLeftNode, visit);
		postorder(currentRightNode, visit);
		visit(treePtr->item);
   }
};
