...
TEST_CASE("Testing insert sort", "[sorted linked list]") {
  ListType lst;
  ...
  REQUIRE(lst.getEntry(3) == 'f');
  REQUIRE(lst.getEntry(4) == 'n');
  REQUIRE(lst.getEntry(5) == 'z');
}
...
TEST_CASE("Testing exceptions", "[sorted linked list]") {
  ...
  CHECK_THROWS_AS(lst.removeAt(6), std::range_error);
